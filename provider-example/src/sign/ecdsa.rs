...
use rustls::{sign::SigningKey, PrivateKey, SignatureAlgorithm, SignatureScheme};
...
impl TryFrom<PrivateKey> for EcdsaSigningKey<p256::ecdsa::SigningKey> {
    type Error = pkcs8::Error;

    fn try_from(value: PrivateKey) -> Result<Self, Self::Error> {
        p256::ecdsa::SigningKey::from_pkcs8_der(&value.0).map(|kp| Self {
            key: Arc::new(kp),
...
impl TryFrom<PrivateKey> for EcdsaSigningKey<p384::ecdsa::SigningKey> {
    type Error = pkcs8::Error;

    fn try_from(value: PrivateKey) -> Result<Self, Self::Error> {
        p384::ecdsa::SigningKey::from_pkcs8_der(&value.0).map(|kp| Self {
            key: Arc::new(kp),
